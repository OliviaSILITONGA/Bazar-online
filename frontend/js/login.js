...
function setRole(role) {
  ...
  document.getElementById("btnBuyer").classList.toggle("active", !isSeller);
  document.getElementById("btnSeller").classList.toggle("active", isSeller);
  document.getElementById("ktmField").style.display = isSeller ? "block" : "none";
  document.getElementById("roleHint").textContent = isSeller
    ? "Akun seller: kamu bisa berjualan di BazarUSU. Wajib upload KTM sebagai verifikasi mahasiswa USU."
    : "Akun pembeli: kamu bisa menjelajahi dan membeli produk.";
  // clear KTM jika ganti ke buyer
  if (!isSeller) resetKtm();
}

// ── KTM upload helpers ─────────────────────────────────────────────────
let ktmFileObj = null;

function ktmSelected(input) {
  const file = input.files[0];
  if (!file) return;
  if (file.size > 5 * 1024 * 1024) {
    toast("❌ Ukuran KTM maksimal 5 MB");
    input.value = "";
    return;
  }
  ktmFileObj = file;
  ...
  reader.onload = (e) => {
    document.getElementById("ktmPreview").src = e.target.result;
    document.getElementById("ktmPreviewWrap").style.display = "block";
    document.getElementById("ktmPlaceholder").style.display = "none";
  };
  reader.readAsDataURL(file);
  showErr("ktmErr", false);
}

function resetKtm() {
  ktmFileObj = null;
  document.getElementById("ktmInput").value = "";
  document.getElementById("ktmPreviewWrap").style.display = "none";
  document.getElementById("ktmPlaceholder").style.display = "block";
  document.getElementById("ktmPreview").src = "";
}

function ktmDragOver(e) {
  e.preventDefault();
  document.getElementById("ktmDrop").classList.add("drag-over");
}

function ktmDrop(e) {
  e.preventDefault();
  document.getElementById("ktmDrop").classList.remove("drag-over");
  const file = e.dataTransfer.files[0];
  if (!file) return;
  const allowed = ["image/jpeg", "image/png", "image/webp"];
  if (!allowed.includes(file.type)) {
    toast("❌ Format KTM harus JPG, PNG, atau WebP");
    return;
  }
  // masukkan ke input file supaya ktmSelected bisa baca
  const dt = new DataTransfer();
  dt.items.add(file);
  const input = document.getElementById("ktmInput");
...
async function doRegister() {
  ...
  // Jika seller, KTM wajib
  if (currentRole === "seller" && !ktmFileObj) {
    showErr("ktmErr", true);
    toast("⚠️ Upload KTM dulu untuk daftar sebagai seller!");
    ok = false;
  } else if (document.getElementById("ktmErr")) {
    showErr("ktmErr", false);
  }
  ...
  try {
    // Kirim sebagai FormData supaya bisa sertakan file KTM
    const formData = new FormData();
    formData.append("name", nama);
    formData.append("email", email);
    formData.append("password", pw);
    if (currentRole === "seller" && ktmFileObj) {
      formData.append("ktm", ktmFileObj);
    }

    const res = await fetch(`${API_URL}/auth/register`, {
      method: "POST",
      credentials: "include",
      body: formData, // JANGAN set Content-Type manual, browser atur sendiri
    });
    ...
    document.getElementById("successSub").textContent = data.user?.is_seller
      ? `Halo ${nama}! Akun seller-mu sudah aktif. Yuk mulai jualan di BazarUSU!`
